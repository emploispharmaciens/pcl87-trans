...
export function FiltersBar({ filters, categories, canCreate, onChange, onCreate }: Props) {
  return (
    <div className="module-panel mb-4 flex flex-wrap items-center gap-2 p-2">
      ...
      >
        <SelectTrigger className="w-36" aria-label="Filtrer par catégorie">
      ...
      >
        <SelectTrigger className="w-32" aria-label="Filtrer par statut">
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
      ...
      {canCreate ? (
        <Button type="button" onClick={onCreate} className="ml-auto">
